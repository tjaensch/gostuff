...
var (
	...
	xslFile    string = "/nodc/users/tjaensch/xsl.git/samos/XSL/ncml2iso_SAMOS_Thomas_edits.xsl"
...
func addCollectionMetadata(ncFile string) {
	...
	var err error
	cmdName := "xsltproc"
	cmdArgs := []string{"--stringparam", "collFile", isocofile, "/nodc/users/tjaensch/xsl.git/samos/XSL/granule.xsl", "./xml_output/" + getFileName(ncFile) + ".xml"}
	if isoXML, err = exec.Command(cmdName, cmdArgs...).Output(); err != nil {
