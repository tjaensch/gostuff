...
var (
	...
	xslFile    string = "/nodc/users/tjaensch/xsl/boulder_demo/go_process/XSL/ncml2iso_modified_from_UnidataDD2MI_demo_WOA_Thomas_edits.xsl"
...
func addCollectionMetadata(ncFile string) {
	...
	var err error
	cmdName := "xsltproc"
	cmdArgs := []string{"--stringparam", "collFile", isocofile, "/nodc/users/tjaensch/xsl/boulder_demo/go_process/XSL/granule.xsl", "./xml_output/" + getFileName(ncFile) + ".xml"}
	if isoXML, err = exec.Command(cmdName, cmdArgs...).Output(); err != nil {
